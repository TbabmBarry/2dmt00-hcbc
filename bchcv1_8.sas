...
/* Check if splited data are balanced */
proc freq data=bchc_race;
	table place*race;
run;
proc freq data=bchc_all;
	table place*race;
run;

/*test ties in data*/
proc rank data=bchc_race out=rankings;
var mortality;
ranks ranks;
run;
...
proc rank data=bchc_all out=rankings;
var mortality;
ranks ranks;
run;
...
/*test ties in log transformed data*/
proc rank data=bchc_race_log out=rankings;
var logm;
ranks ranks;
run;
...
/*test ties in log transformed data*/
proc rank data=bchc_all_log out=rankings;
var logm;
ranks ranks;
run;
...
ods output LSMeans=bchc_logMean;
ods output Diffs=bchc_diff_logMean; /* Store results of Differences of Least Squares Means */
ods output CovParms=bchc_CovParms;
ODS TRACE ON;
proc mixed data=bchc_race_log method=type3 cl;
    class place race;
    model logm = race / solution outp=pred_race;
...
proc print data=pred_race;
run;
proc print data=bchc_CovParms;
run;
proc print data=bchc_logMean;run;
proc print data=bchc_diff_logMean;run;
...
K=7;
use bchc_logMean;
read all var{Effect	race	Estimate	StdErr	DF	tValue	Probt	Alpha	Lower	Upper};
close bchc_logMean;
Mean=Estimate;
raceA=race;
L=Lower;
...
use bchc_diff_logMean;
read all var{race	_race	Estimate	Lower	Upper	AdjLower	AdjUpper};
close bchc_diff_logMean;
diffMean=Estimate;
/* diffRatioL=Lower; */
/* diffRatioU=Upper; */
diffRatioL=AdjLower;
...
use bchc_CovParms;
read all var{CovParm	Estimate	Alpha	Lower	Upper};
close bchc_CovParms;
SigmaG=Estimate[1];
SigmaE=Estimate[2];
/* Back-transformation: the estimated of mortality rate and its CI */
realMean=exp(Mean+(K*SigmaG**2+SigmaE**2)/(2*J*K));
LCL=exp(L+(K*SigmaG**2+SigmaE**2)/(2*J*K));
UCL=exp(U+(K*SigmaG**2+SigmaE**2)/(2*J*K));

/* Back-transformation: the estimated ratio of mortality rate and its CI */
realDiffRatioMean=exp(diffMean+(K*SigmaG**2+SigmaE**2)/(J*K));
diffRatioLower=exp(diffRatioL+(K*SigmaG**2+SigmaE**2)/(J*K));
diffRatioUpper=exp(diffRatioU+(K*SigmaG**2+SigmaE**2)/(J*K));

TMP=realMean||LCL||UCL;
create realMean from TMP[colname={'Estimate_Mean' 'Lower' 'Upper'}];
append from TMP;
close realMean;

create realDiffMean var{raceB raceC realDiffRatioMean diffRatioLower diffRatioUpper};
append;
close realDiffMean;
	
Difference=j(6,1,0);
race1=j(6,1,'AAAAAAAAAA');
race2=j(6,1,'AAAAAAAAAA');
k=1;
Do i=1 to 4;
Do j=i+1 to 4;
Difference[k]=realMean[i]-realMean[j];
race1[k]=raceA[i];
race2[k]=raceA[j];
k=k+1;
end;
end;
create bchc_diffs var{race1 race2 Difference};
append;
close bchc_diffs;
quit;

proc print data=realMean;
run;
proc print data=realDiffMean;run;
proc print data=bchc_diffs;
run;
